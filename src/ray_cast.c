...
double	wall_hit_hori(t_window *data, double angle)
{
	...
	h_x = data->data_player->posx + (h_y - data->data_player->posy)
		/ tan(angle);
...
double	wall_hit_vert(t_window *data, double angle)
{
	...
	v_y = data->data_player->posy
		+ (v_x - data->data_player->posx) * tan (angle);
...
void	ray_casting(t_window *data)
{
	...
	abs = 0;
	data->start_angle = data->data_player->angle
		- (data->data_player->fov_rad / 2);
